...
export default definePlugin({
    ...
    draftRehydrateInterval: null as null | number,
    ...
    start() {
        ...
        this.startDraftRehydratePoll();
    },

    stop() {
        this.stopDraftRehydratePoll();
    ...
    pollCurrentChannelDraftRehydrate() {
        const channelId = SelectedChannelStore.getChannelId?.();
        if (!channelId) return;

        const uploads = safeGet(() => UploadAttachmentStore?.getUploads?.(channelId, DraftType.ChannelMessage)) ?? [];
        const onlyAutoText = uploads.length > 0 && uploads.every(isAutoTextUpload);
        if (!onlyAutoText) return;
        this.rehydrateDraftFromAutoTextUpload(channelId);
    },

    startDraftRehydratePoll() {
        this.stopDraftRehydratePoll();
        const run = () => {
            try {
                this.pollCurrentChannelDraftRehydrate();
            } catch { }
        };

        run();
        this.draftRehydrateInterval = window.setInterval(run, 250);
    },

    stopDraftRehydratePoll() {
        if (this.draftRehydrateInterval != null) {
            clearInterval(this.draftRehydrateInterval);
            this.draftRehydrateInterval = null;
        }
    },
